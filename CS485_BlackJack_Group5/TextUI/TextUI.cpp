...
// Function:    registerEvent
//
// Description: Unregisters an event from the menu by erasing is from the map.
//
// Parameters:  eventName - event menu name
//
// Returned:    None
//***************************************************************************
void TextUI::unRegisterEvent (std::string eventName)
{ 
  auto item = mEventMap.find (eventName);
  mEventMap.erase (item);
}
